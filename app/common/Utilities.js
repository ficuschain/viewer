...
*/
Ext.define('PGP.common.Utilities', {
    statics: {
	
        add: function (a, b) {
            return a + b;
        },
		queryTableAsJson: function(sql){
			var retVal = null;
			Ext.Ajax.request({
				async: false,
				url: '/webapi/api/util/querytableasjson?database=&sql=' + sql,
		...
		applyPermalink:function(map){
		   ...
		   //March 21, 2016 add markers on permalink - Melvin
			if (params.l.length<=0){
					...
					markers.addMarker(new OpenLayers.Marker(new OpenLayers.LonLat(params.g[0],params.g[1]),icon));
